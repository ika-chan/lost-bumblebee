    ...
    if ~isempty(z{i,2})
        for j = 1:length(z{i,2})
            ...
            if any(seen_landmarks == z{i,2}(j)) % old landmark, add a loop-closure
                ...
                l_id = length(seen_landmarks);
                graph.add(BearingRangeFactor2D(symbol('x', i),symbol('l', l_id),Rot2(bearing),range,noiseModel));
                initials.insert(symbol('l', l_id), Point2(x_odom(i,1) + z{i,1}(j,1), x_odom(i,2) + z{i,1}(j,2)));
            end
        end
    end
end
...
for i = 1:np
    initials.insert(symbol('x', i), Pose2(x_odom(i,1), x_odom(i,2), 0));
end
...
plot2DPoints(results, [], marginals);
