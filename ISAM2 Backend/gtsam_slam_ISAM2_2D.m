...
%{
    Author  Dingyi Sun
    Date    April 6 2019
    This program uses the model from Maani's ls_slam.m.
...
initModel = noiseModel.Diagonal.Sigmas([1e-3; 1e-3; 1e-4]);
odomModel = noiseModel.Diagonal.Sigmas([0.2; 0.2; 0.1]);
noiseModel = noiseModel.Diagonal.Sigmas([0.1; 0.06]);   % This comes from guessing. It shouldn't be a gaussian.
% Initial pose

graph.add(PriorFactorPose2(symbol('x', 1), Pose2(0, 0, 0), initModel));
initials.insert(symbol('x', 1), Pose2(x_odom(1,1), x_odom(1,2), 0));
...
if ~isempty(z{1,2})
    ...
    for j = 1:length(z{1,2})
        range = sqrt(z{1,1}(j,1)^2+z{1,1}(j,2)^2);
        bearing = atan2(z{1,1}(j,2),z{1,1}(j,1));
        if any(seen_landmarks == z{1,2}(j)) % old landmark, add a loop-closure
            % find the landmark id stored in A
            l_id = find(seen_landmarks == z{1,2}(j));
            graph.add(BearingRangeFactor2D(symbol('x', 1),symbol('l', l_id),Rot2(bearing),range,noiseModel));
        else % new landmark, exploration;
            % append landmark id
            seen_landmarks = [seen_landmarks, z{1,2}(j)];
            l_id = length(seen_landmarks);
            graph.add(BearingRangeFactor2D(symbol('x', 1),symbol('l', l_id),Rot2(bearing),range,noiseModel));
            initials.insert(symbol('l', l_id), Point2(x_odom(1,1) + z{1,1}(j,1), x_odom(1,2) + z{1,1}(j,2)));
        end
    end
end
isam2.update(graph, initials)
...
poses = get2Dposes(results{1}, marginals);

for i = 2:np
    
    graph = NonlinearFactorGraph;
    initials = Values();
    graph.add(BetweenFactorPose2(symbol('x', i-1), symbol('x', i), Pose2(u(i-1,1), u(i-1,2), 0), odomModel));
    if ~isempty(z{i,2})
        for j = 1:length(z{i,2})
            range = sqrt(z{i,1}(j,1)^2+z{i,1}(j,2)^2);
            bearing = atan2(z{i,1}(j,2),z{i,1}(j,1));
            if any(seen_landmarks == z{i,2}(j)) % old landmark, add a loop-closure
                % find the landmark id stored in A
                l_id = find(seen_landmarks == z{i,2}(j));
                graph.add(BearingRangeFactor2D(symbol('x', i),symbol('l', l_id),Rot2(bearing),range,noiseModel));
            else % new landmark, exploration;
                % append landmark id
                seen_landmarks = [seen_landmarks, z{i,2}(j)];
                l_id = length(seen_landmarks);
                graph.add(BearingRangeFactor2D(symbol('x', i),symbol('l', l_id),Rot2(bearing),range,noiseModel));
                initials.insert(symbol('l', l_id), Point2(poses.cord{i-1}(1) + u(i-1,1) + z{i,1}(j,1), poses.cord{i-1}(2) + u(i-1,2) + z{i,1}(j,2)));
            end
        end
    end
    initials.insert(symbol('x', i), Pose2(poses.cord{i-1}(1) + u(i-1,1), poses.cord{i-1}(2) + u(i-1,2), 0));
    isam2.update(graph, initials)
    tic
    ...
    poses = get2Dposes(results{i}, marginals);
...
plot2DTrajectory(results{index}, [], marginals);
plot2DPoints(results{index}, [], marginals);
